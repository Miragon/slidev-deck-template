 ...
 * chapter-resources plugin (deck/vite.config.ts) strips that background at
 * serve/build time — validated by the sibling excalidraw-built-transparent rule.
 */
const LIGHT = /^#(?:f9f7f7|ffffff)$/i

export const excalidrawCommittedLight = {
  ...
  check() {
    ...
    for (const file of excalidrawSvgFiles()) {
      ...
      if (/\bfilter=/.test(tag)) offenders.push({ file: rel, message: 'dark-mode filter on <svg> (must be light)' })
      ...
      if (dim) {
        ...
        if (!m) offenders.push({ file: rel, message: 'no full-canvas background rect (needs a light <rect> so it previews light)' })
        else if (!LIGHT.test(m[1])) offenders.push({ file: rel, message: `background ${m[1]} is not light (must be #F9F7F7 or #FFFFFF)` })
      }
    }
    return offenders
  },
