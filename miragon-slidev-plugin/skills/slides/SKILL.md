...
1. **Rendering truth (code), what actually ships:**
   - `packages/toolkit/styles/theme.css`: the brand colour tokens as CSS variables, mirrored from the brand's `tokens.json`.
...
**Colours are not owned here.** The source of truth is `tokens.json` in the `miragon-brand` plugin's `corporate-design` skill (`skills/corporate-design/assets/tokens.json`); `theme.css` only mirrors it. So if a value disagrees, **`tokens.json` wins for colours** and a differing `theme.css` is the bug; the layout/component `.vue` wins for its prop signature; and this skill wins for editorial/composition rules.
...
- **Cards are always white.** Use `<Card>` (in a `<CardGrid>` for multiple); never a coloured/gradient tile, never a coloured left-border. Accent goes on the **title only**, and only `accent="blue"` or `accent="blue-mid"`: a card title is text on white, where green fails AA.
...
- **No hardcoded hex** in slide markdown. Use components and CSS variables. (The sanctioned hex lives in `packages/toolkit/styles/theme.css`, mirroring `tokens.json`; components such as `Card.vue` only reference those tokens. Never in a slide.)
...
`Card` replaces hand-written card `<div>`s: background is always white, the accent lands on the title only. `CardGrid` replaces the raw `<div class="grid grid-cols-N …">` wrapper, so the markdown carries no CSS classes. For 2+ cards, alternate the two sanctioned stops left → right.
...
<Card title="DaemonSet" accent="blue-mid">Exactly one pod per node.</Card>

<Card title="CronJob" accent="blue">Starts pods on a schedule.</Card>

</CardGrid>
```

Accent stops: only **`blue`** (`#335DE5`) and **`blue-mid`** (`#2B50D4`) are sanctioned. Alternate them left → right: **2 cards** `blue, blue-mid` · **3 cards** `blue, blue-mid, blue` · **4 cards** `blue, blue-mid, blue, blue-mid`. A card title is text on white, and `#00E676` reaches only 1.67:1 there, so **green is never a card-title colour**; `teal` is not a Miragon colour at all. `teal`, `green-deep`, `green-mid` and `green` survive as deprecated aliases that render as `blue-mid`, so an old deck keeps working, but do not write them in new slides. Green belongs on the gradient accent bar and the bullet markers instead. The stops resolve to the `--miragon-card-accent-*` tokens in `theme.css`; never write a hex in markdown.
...
`alt` is not a nice-to-have: a visual that carries content needs a short descriptive alt text (what it shows, not "diagram"), and a purely decorative visual needs an explicit empty `alt=""` so screen readers skip it. `Figure` defaults `alt` to `""`, so leaving it off silently marks a content-carrying diagram as decorative. That is the failure mode to avoid.
...
**Typography in markdown:** headings black (`#1D1D1D`, the CI black, never `#000000`); blue only for kickers/accents/labels; never set `font-family`. The accent bar is the one brand moment per slide, so don't add extra bars.

**Punctuation / emoji / bullets:** no em-dashes; no emoji (inline SVG or a Lucide Iconify class, `i-lucide-*`, never a second icon set); plain `<ul><li>` with the layout's marker, never per-slide list styling.

**Focal point:** one clear focal point the eye lands on first (a big number, a bold lead, a dominant visual). One bold lead line, and green at most once per slide, as a surface or graphic (the gradient bar, a marker, one diagram node), never as coloured text. Scale, don't multiply: fewer elements, larger, full slide width.
...
**Voice:** calm, confident, modern. No exclamation marks, no marketing fluff. Green is the accent of last resort: at most one green element per slide, for the key payoff, and carried by a surface rather than by type.
...
npm run verify:source  # fast: source-level guardrails only, no dev server or browser
npm run verify         # full: source + rendered checks (boots Slidev + Chromium)
npm run verify:ci      # same as verify, headless text report, use this in CI
node scripts/shot.mjs <page> [base]   # screenshot one rendered slide (auto-detects the portless dev URL)
```

The guardrails are the versioned **`@miragon/slidev-validator`** package (an exact-pinned dependency), driven by `slidev-validator`. Rules have stable ids and severities (`off`/`warn`/`error`); `slidev-validator.config.mjs` (extends `@miragon/slidev-validator/recommended`) is where a deck tunes or scopes them. Never disable a rule to make a slide pass; fix the slide.

`npm run build` only proves the Markdown/Vue **compiles**: it says nothing about whether a slide overflows or breaks a design rule. For that, `npm run verify` runs the rendered checks against the live, fully-revealed slides and measures the DOM-checkable rules: fits the canvas with a bottom margin (content must clear the 552px floor by >= 16px, not merely fit), clears the global overlays (no content collides with or crowds the page/chapter display or the progress bar, see [Global overlays and safe areas](#global-overlays-and-safe-areas)), no em-dashes, no emoji, headings black, cards white, no inline fonts, no restyled or nested bullets. It also runs source-level checks: **every slide declares a sanctioned layout** (each slide's `layout:` must be a theme archetype or the built-in `default`; `src:` stubs are exempt), **no raw HTML** (slide bodies are markdown + components only; tags like `<div>`/`<span>` are flagged, while code fences and comments are exempt so HTML can be shown as an example), **no HTML entities in the slide source** (write the literal character, never `&#39;`/`&amp;`/…), **content headings stay single-line** (no `<br>`/newline/trailing `\` in a content layout's `title:`, and within a per-layout character budget; opt out one slide with `allowMultilineHeading: true`) and **every `.excalidraw.svg` is light and transparent** (no dark-mode filter, no baked white background). These source-level checks run fast on their own via `npm run verify:source` (no dev server) and gate CI so forks inherit them. `npm run verify` additionally runs the rendered checks and prints a text report that lists each rule's status plus any deliberately disabled rules and exceptions.

The rendered run defaults to port **3030** (override with `VERIFY_PORT=<port>`). If nothing is on 3030 it starts its own fresh server for `deck/slides.md`; if a server is already there it **reuses** it. Full rule reference, severities and config schema: [`packages/validator/README.md`](../../../packages/validator/README.md).
...
- **Never free a port or kill a process**: no `pkill -f slidev`, no `kill $(lsof -t -i:PORT)`, no `fuser -k`. A busy dev port is the author's server doing its job; leave it. (The verify HTML-report viewer from `npm run verify` is the one exception: it blocks, so prefer `npm run verify:ci`, which never opens it.)
...
- **`BrandMeshBackground`** (`packages/toolkit/components/BrandMeshBackground.vue`): colour stops `#335DE5, #1A1A4E, #00E676, #335DE5, #0D0D2B, #00C853`; DISTORTION 1, SWIRL 0.5, SPEED 0.6. Corporate identity. Used by `cover` and `closing` only, and the three off-palette stops live nowhere else.
- **Brand colours** come from `tokens.json` in the `miragon-brand` plugin's `corporate-design` skill; `packages/toolkit/styles/theme.css` only mirrors them. Primary blue `#335DE5`, sparing green `#00E676` (surfaces and graphics, never text on white), CI black `#1D1D1D`. No orange, no teal, no other primaries.
- **The logo files** in `packages/toolkit/assets/` (`logo.svg` green, `logo-white.svg`, `logo-blue.svg`, `komet.svg`) are the unaltered official vectors. Never recolour, redraw, distort, or add effects, `opacity` included. Variant by ground: light → green or blue, dark/coloured or photo → white, monochrome → black. Clearspace is the height of the "M"; minimum size 96px wordmark, 24px komet. `cover` and `closing` use the white variant because they sit on the mesh.
- **Icons are Lucide** (`i-lucide-*`), outline, on the 24px grid. One set for the whole deck, never mixed with another family.
...
To change structure: edit the relevant `packages/toolkit/layouts/*.vue`. To change a colour: change `tokens.json` in the brand skill first, then mirror it into `packages/toolkit/styles/theme.css`, never the other way round.
