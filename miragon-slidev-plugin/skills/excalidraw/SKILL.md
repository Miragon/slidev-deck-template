...
clicked to edit as the box's label. The render hides this; the scene shows it. The fix, which is how the
...
the label's explicit coordinates, because binding without coordinates collapses the label to the origin.
...
- **Leaf box (no children) → one centred bound label.** A title + subtitle is **ONE multi-line bound
  ...
  a two-tone title/subtitle collapses them to one uniform style, which is expected and correct
...
The Miragon palette, nothing else. The source of truth is `tokens.json` in the `miragon-brand` plugin's
`corporate-design` skill, mirrored by `packages/toolkit/styles/theme.css`. **Do not invent colours**, and
never reach for an Excalidraw default swatch: its orange/yellow/red/slate presets are off-brand.
Four values carry every diagram:

| Use | Value |
|---|---|
| Strokes, frames, arrows, emphasis | `#335DE5` blue |
| Fills and surfaces | `#FFFFFF` white, `#F9F7F7` grey |
| Text: labels, kickers, captions | `#1D1D1D` CI black (never `#000000`) |
| Accent, sparing, surface or shape only, never text | `#00E676` green |

Node roles are told apart by **fill and line**, not by extra hues (the schema reference's rule: use width
for emphasis, not colour noise):

| Role | `backgroundColor` | `strokeColor` | Line | Label text |
|---|---|---|---|---|
| **solid**, component, emphasis | `#FFFFFF` | `#335DE5` | solid, `strokeWidth` 2 | `#1D1D1D` |
| **accent**, highlighted node | `#F9F7F7` | `#335DE5` | solid, `strokeWidth` 2 | `#1D1D1D` |
| **frame**, zone / container | `#F9F7F7` | `#335DE5` | solid, `strokeWidth` 1 | free kicker `#1D1D1D` |
| **plain**, sub-node inside a frame | `#FFFFFF` | `#335DE5` | solid, `strokeWidth` 1 | `#1D1D1D` |
| **dashed**, virtual boundary | `#FFFFFF` | `#335DE5` | `strokeStyle: "dashed"` | `#1D1D1D` |

**Text colours:** `#1D1D1D` for every label, kicker and caption. `#335DE5` only when the label *is* the
emphasis (the brand label on a solid node). Never `#000000`, and never green: `#00E676` on white is
1.67:1 and unreadable.

**Green is the accent of last resort**, at most one node per diagram, only for the key success payoff,
and only as a **surface**: fill `#00E676` with the label still `#1D1D1D` on top. No green text, no green
stroke on ordinary boxes, no orange, no teal, no other primaries.
...
- **`fontFamily: 2` (Helvetica) on EVERY text**, the closest match to the brand's Geist. **Never `1` (Virgil,
...
- **Box labels: own label, placed by content.** A box's text is the box's own label. If the box has **no
  ...
  **top-left** as a free kicker (inset to clear the corner), because Excalidraw centres a bound label, so a corner
...
forces light mode; `-b true` bakes the `#F9F7F7` background. **Always pass `-d false`**, because the tool's
...
> are explicit in the file, so grep the output for the palette hexes to confirm; re-export if a dark fill
> crept in.
...
**`alt` is mandatory here.** A diagram always carries content, so describe what it shows in a short
phrase (not "diagram", not the filename). An empty `alt=""` is reserved for purely decorative visuals,
which a diagram never is.
...
- a one-way / dangling binding, where a box lists a text in `boundElements` but the text has no `containerId`
...
Fix in place, and **do not globally re-sort** to "tidy" (that hides fills over content). Re-export and verify.
